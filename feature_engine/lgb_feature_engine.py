...
class LgbFeatureEngine(BaseFeatureEngine):
    ...
    def post_process(self,info_dict, train_test = "train"):
        ...
        if not (label is None) and train_test == "train":
            ...
            label_clip_index = label_clip_index.reshape(-1,label.shape[-1])

            assert feature.shape[0] == label.shape[0]

        if train_test == "train":
            feature = feature[re_label_valid_index]
            label = label[re_label_valid_index]
            label_clip_index = label_clip_index[re_label_valid_index]
            info_dict["label_clip_index"] = label_clip_index
